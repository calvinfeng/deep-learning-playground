...
class PatchMerging(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, downscaling_factor: int):
        ...
        self.patch_merge = nn.Unfold(
            kernel_size=downscaling_factor, stride=downscaling_factor)
...
class PreNorm(nn.Module):
    def __init__(self, embed_dim: int, fn: Callable):
...
class WindowAttention(nn.Module):
    def __init__(
        self, embed_dim: int, num_heads: int, head_dim: int, shifted: bool, window_size: int,
        relative_pos_embedding: bool,
    ):
        """Perform self-attention on each window of the input. If shifted is True, the window is
        shifted by half of the window size. This allows "global attention" across stages. In
        essence, shifted window attention combines the ideas of local attention and global
        attention.
    ...
    def forward(self, x):
        ...
        q, k, v = map(
            lambda t: rearrange(t, eqn, h=num_heads, win_h=window_size, win_w=window_size),
            qkv
        )
        ...
        q_dot_k = einsum("b h w i d, b h w j d -> b h w i j", q, k) * self.scale

        if self.relative_pos_embedding:
            q_dot_k += self.pos_embedding[
        ...
        if self.shifted:
            q_dot_k[:, :, -num_win_w:] += self.upper_lower_mask
            q_dot_k[:, :, num_win_w - 1::num_win_w] += self.left_right_mask
...
class SwinBlock(nn.Module):
    def __init__(
        self, embed_dim: int, num_heads: int, head_dim: int, mlp_dim: int, shifted: bool, window_size: int,
        relative_pos_embedding: bool,
    ):
        """_summary_
        ...
        self.mlp_block = Residual(
            PreNorm(embed_dim, FeedForward(
                input_dim=embed_dim, hidden_dim=mlp_dim)),
        )
...
class StageModule(nn.Module):
    def __init__(self, in_channels: int, hidden_dim: int, layers: int, downscaling_factor: int, num_heads: int,
        ...
        self.patch_partition = PatchMerging(
            in_channels=in_channels, out_channels=hidden_dim, downscaling_factor=downscaling_factor
        )
